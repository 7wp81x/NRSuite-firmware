...
public:
    ...
    // ─── Mouse (combined HID report) ──────────────────────────────────────
    void mouseMove(int8_t dx, int8_t dy);
    void mouseScroll(int8_t wheel);
    void mouseButton(uint8_t buttonMask, bool down);
    void mouseClick(uint8_t buttonMask);
...
private:
    ...
    uint8_t  _mouseReport[HID_MOUSE_REPORT_SIZE];   // [buttons][dx][dy][wheel]
