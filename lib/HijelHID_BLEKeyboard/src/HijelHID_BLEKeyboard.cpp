...
void HijelHID_BLEKeyboard::mouseMove(int8_t dx, int8_t dy) {
    if (!_connected) return;
    if (_pendingIdleTransition) {
        _transitionToActive();
    } else if (_connState == _ConnState::Active) {
    ...
    _mouseReport[1] = (uint8_t)dx;
    _mouseReport[2] = (uint8_t)dy;
    _sendMouseReport();
}

void HijelHID_BLEKeyboard::mouseScroll(int8_t wheel) {
    if (!_connected) return;
    if (_pendingIdleTransition) {
        _transitionToActive();
    } else if (_connState == _ConnState::Active) {
    ...
    _mouseReport[3] = (uint8_t)wheel;
    _sendMouseReport();
}

void HijelHID_BLEKeyboard::mouseButton(uint8_t buttonMask, bool down) {
    if (!_connected) return;
    if (_pendingIdleTransition) {
        _transitionToActive();
    } else if (_connState == _ConnState::Active) {
    ...
    if (down) {
        _mouseReport[0] |= buttonMask;
    } else {
        _mouseReport[0] &= (uint8_t)~buttonMask;
    }
    _sendMouseReport();
}

void HijelHID_BLEKeyboard::mouseClick(uint8_t buttonMask) {
    mouseButton(buttonMask, true);
    delay(30);
    mouseButton(buttonMask, false);
}

void HijelHID_BLEKeyboard::mouseReleaseAll() {
    memset(_mouseReport, 0, sizeof(_mouseReport));
    if (_connected) {
        _sendMouseReport();
    }
...
void HijelHID_BLEKeyboard::_sendMouseReport() {
    if (_pMouseInput == nullptr || !_connected) return;
    _logVf("mouseReport: buttons=0x%02X dx=%d dy=%d wheel=%d",
           _mouseReport[0], (int8_t)_mouseReport[1],
           (int8_t)_mouseReport[2], (int8_t)_mouseReport[3]);

    _pMouseInput->setValue(_mouseReport, HID_MOUSE_REPORT_SIZE);
    int retries = 0;
    ...
    // Movement/scroll are one-shot; keep button state but clear deltas.
    _mouseReport[1] = 0;
    _mouseReport[2] = 0;
    _mouseReport[3] = 0;
}
