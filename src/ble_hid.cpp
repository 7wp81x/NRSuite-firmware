...
static uint8_t mouseButtonMask(const String& button) {
    String name = button;
    name.toUpperCase();
    name.trim();
    if (name == "LEFT")   return 0x01;
    if (name == "RIGHT")  return 0x02;
    if (name == "MIDDLE") return 0x04;
    if (name == "BACK")   return 0x08;
    if (name == "FORWARD")return 0x10;
    return 0;
}

void mouseMove(int dx, int dy) {
    if (!isConnected() || !keyboard) return;
    keyboard->mouseMove((int8_t)constrain(dx, -127, 127),
                        (int8_t)constrain(dy, -127, 127));
}

void mouseScroll(int wheel) {
    if (!isConnected() || !keyboard) return;
    keyboard->mouseScroll((int8_t)constrain(wheel, -127, 127));
}

void mouseButton(const String& button, bool down) {
    if (!isConnected() || !keyboard) return;
    uint8_t mask = mouseButtonMask(button);
    if (mask != 0) keyboard->mouseButton(mask, down);
}

void mouseReleaseAll() {
    if (!isConnected() || !keyboard) return;
    keyboard->mouseReleaseAll();
}
